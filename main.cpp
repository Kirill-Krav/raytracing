...
Vec cast_ray(const Vec &orig, const Vec &dir, const std::vector<Sphere> &spheres, const std::vector<Light> &lights, int depth = 0)
{
    ...
    if (depth < 10 && scene_intersect(orig, dir, spheres, material, point, N))
    {
        ...
        return res + reflect_color * material.getReflection();
    }
    else
    {
...
int main()
{
    ...
    // spheres.push_back(Sphere(Vec(-1.0f, -1.5f, -12.0f), 2.0f, mirror));
    ...
    spheres.push_back(Sphere(Vec(0, 2, -40), 14, mirror));
    spheres.push_back(Sphere(Vec(16, 5, -41), 9, mirror));
